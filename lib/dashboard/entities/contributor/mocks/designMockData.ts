...
export function getContributorDesignData(repoId: string, contributorCount: number = 6): ContributorDesignRow[] {
  ...
  return baseContributors.map((contributor, index) => {
    // Use contributor name and index as deterministic seed
    const seed = contributor.contributorName.charCodeAt(0) + contributor.contributorName.charCodeAt(1) * 10 + index * 1000;
...
export function transformToOwnershipScatterData(contributors: ContributorDesignRow[]): DeveloperPoint[] {
  ...
  return contributors.map((contributor) => {
    ...
    const kpZScore = Math.abs((contributor.totalKarmaPoints - meanKp) / stdKp);
    const ownZScore = Math.abs((contributor.ownershipPct - meanOwn) / stdOwn);
    const inNormalRange = kpZScore <= 1 && ownZScore <= 1;

    // Determine outlier type based on ownership
    let outlierType: "high" | "low" | null = null;
    if (!inNormalRange) {
      outlierType = contributor.ownershipPct > meanOwn ? "high" : "low";
    }

    return {
      name: contributor.contributorName,
      team: contributor.repoId,
      totalKarmaPoints: contributor.totalKarmaPoints,
      ownershipPct: contributor.ownershipPct,
      inNormalRange,
      outlierType,
    };
  });
...
export function transformToChaosMatrixData(
  ...
  return contributors.map((contributor, index) => {
    ...
    const baseKp = contributor.medianWeeklyKp;
    const baseChurn = contributor.churnRatePct;
    ...
    return {
      name: contributor.contributorName,
      team: contributor.contributorName, // Use contributor name as "team" so each contributor is a distinct group
