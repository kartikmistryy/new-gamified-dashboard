...
import type { RepoContributionFlow, ContributionNode, ContributionLink } from "../../mocks/spofContributionData";
...
/**
 * Calculate flow layout for Repository Contribution Flow (Contributors → Modules).
 * Adapted from team dashboard calculateFlowLayout().
 */
export function calculateFlowLayout(
  flow: RepoContributionFlow,
  width: number,
  height: number
): FlowLayout | null {
  if (flow.nodes.length === 0 || flow.links.length === 0) return null;

  const contributorNodes = flow.nodes.filter((node) => node.side === "contributor");
  const moduleNodes = flow.nodes.filter((node) => node.side === "module");
  if (contributorNodes.length === 0 || moduleNodes.length === 0) return null;
  ...
  const contributorTotal = contributorNodes.reduce((sum, node) => sum + node.value, 0);
  const moduleTotal = moduleNodes.reduce((sum, node) => sum + node.value, 0);
  const totalValue = Math.max(contributorTotal, moduleTotal, 1);

  const maxNodeCount = Math.max(contributorNodes.length, moduleNodes.length);
